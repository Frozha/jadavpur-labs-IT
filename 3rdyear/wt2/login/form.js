function validate(){
  ...
  if (!loginPattern.test(login)) {
      document.getElementById("error").innerHTML = "Login name must be alphanumeric and can include underscores. It should not exceed 10 characters.";
  ...
  if (password.length < 6 || password.length > 10) {
      document.getElementById("error").innerHTML = "Password must be between 6 and 10 characters.";
      document.getElementById("error").style.color = "red";
      return false; 
  }
  ...
  if (!digitPattern.test(password) || !uppercasePattern.test(password)) {
      document.getElementById("error").innerHTML = "Password must contain at least one digit and one uppercase letter.";
      document.getElementById("error").style.color = "red";
      return false;  
  }
